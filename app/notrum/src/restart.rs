...
//! it starts the installed Notrum executable directly, without a shell, and
//! keeps the new UI from opening until the old UI has finished shutting down.

use notrum_update::{InstallKind, Installation};
use std::io::{self, Read, Write};
use std::path::Path;
use std::process::{Child, ChildStdin, Stdio};

pub(crate) const HANDOFF_FLAG: &str = "--restart-after-update";
const READY: &[u8] = b"notrum/restart/ready";
...
impl PendingRestart {
    pub(crate) fn start(installation: &Installation, workspace: Option<&Path>) -> io::Result<Self> {
        ...
        let executable = match installation.kind() {
            InstallKind::MacApp => installation.target().join("Contents/MacOS/Notrum"),
...
mod tests {
    ...
    fn child_command(root: &Path) -> std::process::Command {
        ...
        command
            ...
            .env("NOTRUM_RESTART_PROBE", root);
        command
    }

    #[test]
    fn child_handoff() {
        let Some(root) = std::env::var_os("NOTRUM_RESTART_PROBE") else {
            return;
        };
    ...
    fn child_waits_for_completion_and_cancel_never_restarts() {
        let root = crate::test_support::workspace("notrum-restart");
    ...
    fn missing_executable_reports_an_error_before_shutdown() {
        let root = crate::test_support::workspace("notrum-restart-missing");
