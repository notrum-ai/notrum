...
.PHONY: all help check clean build build-container native-smoke native-external-smoke demo-data test-demo-data check-macos test test-release lint fmt fmt-check lock tree audit audit-source audit-dependencies audit-vulnerabilities \
...
check: fmt-check lint test test-release test-demo-data check-macos package-macos-smoke ui-check audit diff-check
